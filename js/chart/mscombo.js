define(['jquery', 'd3', 'chartBase'], function($, d3, Chart) {
    ...
    MSComboDualY.prototype.syncAxes = function(data) {
        ...
        if(no_data || !x_min_max) {
            ...
            if (y1min < 5) {
                y1min = Math.floor(y1min);
            }
            if (y1max < 5) {
                y1max = Math.ceil(y1max);
            }

            // As part of https://exceleron.myjetbrains.com/youtrack/issue/P3-4045,
            // commenting out the code for aligning the axes.

            // Unlike the column chart we are not throwing error for undefined min and max.
            // This is based on the assumption that line chart will be only for secondary data.
            // Hence it is fine to render zero values for this chart.


            //Code credit: http://jsfiddle.net/jglover/XvBs3/3/

            /* var y1min = x_min_max[0];
            var y1max = x_min_max[1];
            var y2min = sec_y_extent[0];
            var y2max = sec_y_extent[1];
            ...
            if(y1min > 0 && y1max > 0 && y2min > 0 && y2max > 0) {
                ...
                y1min = 0;
                y2min = 0;
            } else if(y1min < 0 && y1max < 0 && y2min < 0 && y2max < 0) {
                // if all numbers are negative set ceiling to zero
                allSameSign = true;
                y1max = 0;
                y2max = 0;
            } else if(y1min === 0 && y1max === 0 && y2min === 0 && y2max === 0) {
                // if all numbers are equal to zero set min = 0 and max =100
                y1min = y2min = 0;
                y1max = y2max = 100;
                allSameSign = true;
            } else {
            ...
            if((y1min < 0 && y1max > 0) || (y1min > 0 && y1max < 0)) {
                if(y2min <= 0 && y2max <= 0) {
                    y2max = 100;
                }
            }

            if((y2min < 0 && y2max > 0) || (y2min > 0 && y2max < 0)) {
                if(y1min <= 0 && y1max <= 0) {
                    y1max = 100;
                }
            }
            ...
            //Setting it positive
            y1ratio = Math.abs(y1ratio);
            y2ratio = Math.abs(y2ratio);
